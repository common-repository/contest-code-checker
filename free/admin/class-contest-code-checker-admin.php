...
class CCC_Contest_Code_Checker_Admin {
	...
	public function register_menus() {
		add_menu_page(__("ContestsWP", "contest-code"),
					  ...
					  plugins_url( 'free/public/images/contestswp_icon_16x16.png', CCC_FREE_PLUGIN_FILE )
					);
	 ...
	 */
	 public function wireup_settings() {
		...
		add_settings_section( 'contest_code_checker_contest_form_options', __( 'Front-End Contest Form Settings', 'contest-code'), null, 'ccc_options' );
		add_settings_field("ccc_display_popover",
											...
											"ccc_options",
											"contest_code_checker_contest_form_options");
		...
		add_settings_field('ccc_contest_not_running',
							__( 'Message for when the contest is not running', 'contest-code' ),
		...
		add_settings_field( 'ccc_do_not_save_invalid',
							__( 'Do not save invalid contest entries', 'contest-code' ),
							array( $this, 'display_do_not_save_invalid' ),
							'ccc_options',
							'contest_code_checker_contest_form_options' );

		add_settings_section( 'contest_code_checker_results_options', __( 'Results Messages', 'contest-code'), null, 'ccc_options' );
		add_settings_field("ccc_text_winning",
											 __("Text for When Someone Wins", "contest-code"),
											 array($this, "display_text_winning_field"),
											 "ccc_options",
											 "contest_code_checker_results_options");
		...
		add_settings_section( 'contest_code_checker_email_options', __( 'Email Notification Settings', 'contest-code'), null, 'ccc_options' );
		add_settings_field('ccc_email_winner',
							...
							"ccc_options",
							"contest_code_checker_email_options");
		...
		register_setting( 'contest_code_checker_options', 'ccc_do_not_save_invalid' );
	}
	...
	public function display_start_date_field($args) {
		$option_value = get_option( 'ccc_start_date' );
		?>
			<input type="text" name="ccc_start_date" id="ccc_start_date" value="<?php echo ( '' === $option_value ) ? '' : date( 
				'm/d/Y', 
	...
	public function display_end_date_field($args) {
		$option_value = get_option( 'ccc_end_date' );
		?>
			<input type="text" name="ccc_end_date" id="ccc_end_date" value="<?php echo ( '' === $option_value ) ? '' : date(
				'm/d/Y', 
				strtotime( $option_value ) ); ?>" />
		<?php
	}
	...
	public function display_do_not_save_invalid( $args ) {
		?>
			<input type="checkbox" name="ccc_do_not_save_invalid" id="ccc_do_not_save_invalid" value="Y" <?php echo ( get_option( 'ccc_do_not_save_invalid' ) == 'Y' ) ? 'checked="checked"' : ''; ?> />
		<?php
	}
		...
		if ( $contestants->have_posts() ) {
			while ( $contestants->have_posts() ) {
				...
				if( $cc->get_prize() != '' ) {
					...
					$csv .= "\"".$cc->get_code()."\",\"".str_replace("\"", "\"\"",$cc->get_prize())."\",\"" . wp_date(
